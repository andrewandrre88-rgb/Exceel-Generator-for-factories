...
export const SupplierResponseView: React.FC<SupplierResponseViewProps> = ({
  ...
  return (
    <div className="bg-slate-100 min-h-screen pb-16">
      {/* Top Banner */}
      <div className="bg-amber-600 text-white px-3 sm:px-4 py-2.5 shadow-xs sticky top-0 z-30 flex items-center justify-between gap-2">
        <div className="flex items-center gap-2 text-xs font-semibold min-w-0">
          <button
            type="button"
            onClick={onClose}
            className="flex items-center gap-1 bg-amber-700 hover:bg-amber-800 px-2 sm:px-2.5 py-1 rounded transition-colors text-white shrink-0"
          >
            <ArrowLeft className="w-3.5 h-3.5" /> <span className="hidden sm:inline">Back to Editor</span><span className="sm:hidden">Back</span>
          </button>
          <span className="truncate hidden md:inline">
            ★ Factory Sourcing View (供应商在线报价模式) - Yellow cells are for your factory quotation
          </span>
          <span className="truncate md:hidden text-[11px]">
            Factory Quotation Mode
          </span>
        </div>
        ...
        <div className="flex items-center gap-1.5 sm:gap-2 shrink-0">
          <button
            ...
            className="hidden sm:flex items-center gap-1 bg-amber-700/80 hover:bg-amber-800 text-xs px-2.5 py-1 rounded font-medium transition-colors"
          >
          ...
          <button
            ...
            className="flex items-center gap-1 sm:gap-1.5 bg-emerald-600 hover:bg-emerald-700 text-xs px-2.5 sm:px-3 py-1 rounded font-bold shadow-xs transition-colors"
          >
      ...
      <div className="max-w-6xl mx-auto px-3 sm:px-4 pt-4 sm:pt-6 space-y-4 sm:space-y-6">
        {/* RFQ Header Card */}
        <div className="bg-white rounded-xl shadow-xs border border-slate-200 p-4 sm:p-6">
          <div className="flex flex-col md:flex-row md:items-start justify-between gap-4 border-b border-slate-100 pb-5">
            ...
            <div className="bg-slate-50 border border-slate-200 rounded-lg p-3 text-xs space-y-1">
              ...
              {metadata.enableExchangeRate && (
                <div>
                  <span className="text-slate-400 font-medium">FX Benchmark:</span>{' '}
                  <span className="font-semibold text-emerald-700 font-mono">1 USD = ¥{metadata.exchangeRateUsdToCny || 7.25}</span>
                </div>
              )}
            </div>
          </div>
